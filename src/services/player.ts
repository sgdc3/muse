...
export default class {
  ...
  private async getStream(song: QueuedSong, options: {seek?: number; to?: number} = {}): Promise<Readable> {
    ...
    type VideoFormatType = VideoInfoType['formats'][number] & {
      asr: number | null;
      abr: number | null;
    };

    let format: VideoFormatType | undefined;
    ...
    if (!ffmpegInput) {
      ...
      const filter = (format: VideoFormatType): boolean => format.acodec === 'opus' && format.ext === 'webm' && format.asr !== undefined && format.asr === 48000;

      format = formats.find(filter);

      const nextBestFormat = (formats: VideoFormatType[]): VideoFormatType | undefined => {
        if (formats.length < 1) {
          return undefined;
        }
        ...
        if (info.media_type === 'livestream') {
          ...
          return formats.find(format => [128, 127, 120, 96, 95, 94, 93].includes(parseInt(format.format_id, 10))); // Bad typings
        }
      ...
      shouldCacheVideo = info.media_type !== 'livestream' && info.duration < MAX_CACHE_LENGTH_SECONDS && !options.seek;
